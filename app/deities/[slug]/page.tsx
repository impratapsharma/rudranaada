...
export default async function Page({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const d=deityHubs.find(x=>x.slug===slug);const profile=deityProfiles[slug];if(!d||!profile)notFound();const related=fullSongs.filter(m=>m.themes.some(t=>t.toLowerCase()===d.name.toLowerCase()||t.toLowerCase()===d.slug));return <>
