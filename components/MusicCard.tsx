...
export function MusicCard({entry:m}:{entry:MusicEntry}) {
  return <Link className="card musicCard" href={'/music/'+m.slug}>
    ...
    <div className="musicCardBody"><div className="cardMeta">{m.eyebrow}</div><h3>{m.title}</h3><p>{m.summary}</p><time className="muted" dateTime={m.publishedAt}>{formatDate(m.publishedAt)}</time></div>
  </Link>;
}
